...
#The location where the AWS_config file is
	aws_config_file = 's3://360inbcu/kochava/Config/nbcu_config.json'
...
class AWS(object):
 ...
 #       ''' Initialize the AWS class with config'''     
		...
		if aws_config_file == None:
			...
			# Path for install temp folder 
			self.install_dataStagingPath = "%s/" % (config['s3Staging']['install_path'].rstrip("/"))
			if not self.dataStagingPath.startswith("s3://"):
				print "install_s3Staging.path must be a path to S3"
				sys.exit(-1)	
			...
			self.install_dest_table = self.destConfig['install_tableName']
			self.install_dest_staging_table = self.destConfig['install_staging_tableName']
	...
	def upsert_install_data(self,conn):
		print '\n' + "=============================================================================================" + '\n'
		print "Upserting to Target Table" + '\n'
		print "Upserting %s.%s from %s" % (self.dest_schema, self.install_dest_table, self.install_dataStagingPath)
		...
		upsert_stmt = """ -- Start 
							...
							SET 
								...
								install_matched_on	=	s.install_matched_on,
								install_post_status	=	s.install_post_status,
								install_date_utc	=	s.install_date_utc,
								install_date_adjusted	=	s.install_date_adjusted,
								install_kochava_device_id	=	s.install_kochava_device_id,
								install_matched_by	=	s.install_matched_by,
								install_click_new_id	=	s.install_click_new_id,
								install_click_date_utc	=	s.install_click_date_utc,
								install_click_date_adjusted	=	s.install_click_date_adjusted,
								install_impression_id	=	s.install_impression_id,
								install_impression_date_utc	=	s.install_impression_date_utc,
								install_impression_date_adjusted	=	s.install_impression_date_adjusted,
								attribution_tracker	=	s.attribution_tracker,
								attribution_tracker_name	=	s.attribution_tracker_name,
								attribution_network	=	s.attribution_network,
								attribution_network_name	=	s.attribution_network_name,
								attribution_site	=	s.attribution_site,
								attribution_creative	=	s.attribution_creative,
								install_original_request	=	s.install_original_request,
								install_duplicate_click	=	s.install_duplicate_click,
								install_price	=	s.install_price,
								install_device_version	=	s.install_device_version,
								install_devices_ids	=	s.install_devices_ids,
								install_devices_idfa	=	s.install_devices_idfa,
								install_devices_android_id	=	s.install_devices_android_id,
								install_devices_adid	=	s.install_devices_adid,
								install_devices_custom	=	s.install_devices_custom,
								install_devices_waid	=	s.install_devices_waid,
								install_devices_ip	=	s.install_devices_ip,
								install_devices_email_0	=	s.install_devices_email_0,
								install_devices_email_1	=	s.install_devices_email_1,
								install_san_original	=	s.install_san_original,
								installgeo_continent_code	=	s.installgeo_continent_code,
								installgeo_country_code	=	s.installgeo_country_code,
								installgeo_country_code3	=	s.installgeo_country_code3,
								installgeo_country_name	=	s.installgeo_country_name,
								installgeo_region	=	s.installgeo_region,
								installgeo_postal_code	=	s.installgeo_postal_code,
								installgeo_dma_code	=	s.installgeo_dma_code,
								installgeo_city	=	s.installgeo_city,
								installgeo_area_code	=	s.installgeo_area_code,
								installgeo_longitude	=	s.installgeo_longitude,
								installgeo_latitude	=	s.installgeo_latitude

							FROM %s.%s s 
							WHERE %s.%s.install_date_utc = s.install_date_utc and %s.%s.install_kochava_device_id = s.install_kochava_device_id; 

							-- Insert records 
							INSERT INTO %s.%s 
							SELECT s.* FROM %s.%s s LEFT JOIN %s.%s
							ON s.install_kochava_device_id = %s.%s.install_kochava_device_id and s.install_date_utc = %s.%s.install_date_utc
							WHERE %s.%s.install_date_utc IS NULL or %s.%s.install_kochava_device_id IS NULL  ;
		...
		try:
			conn.query(upsert_stmt % (#Create a staging table 
									  self.dest_schema, self.install_dest_staging_table,
									  self.dest_schema, self.install_dest_table,
									  #Load data into the staging table 
									  self.dest_schema, self.install_dest_staging_table,
									  self.install_dataStagingPath, 
									  self.accessKey, self.secretKey,
									  #Update records 
									  self.dest_schema, self.install_dest_table,
									  self.dest_schema, self.install_dest_staging_table,
			...
			print '\n' + "Successfully upserted %s.%s from %s" % (self.dest_schema, self.install_dest_table, self.install_dataStagingPath)
		except Exception as e:
			print '\n'+'Had error upserting data to the Redshift'
			print logging.warning(e)
			sys.exit(-1)
	...
	def s3Delete(self):
		print '\n' + "=============================================================================================" + '\n'
		print "Start cleaning up S3 Data Temp Location %s and %s" % (self.dataStagingPath,self.install_dataStagingPath)
		s3Info = self.tokeniseS3Path(self.dataStagingPath)
		install_s3Info = self.tokeniseS3Path(self.install_dataStagingPath)
		...
		print '\n'+'Successfully delete ',i,' files in S3 Data Temp Location'

		j = 0
		for key in stagingBucket.list(install_s3Info[1]):
			stagingBucket.delete_key(key)
			j = j + 1

		print '\n'+'Successfully delete ',j,' files in Install S3 Data Temp Location'
	...
	def install_upload2S3(self,filenames,local_path):
		print '\n' + "=============================================================================================" + '\n'
		print "Start uploading install CSV into S3"
		...
		if type(filenames) == str:         # single file (from webpage)
			local_filename = os.path.join(local_path, filenames)
			try:	
				s3.upload_file(local_filename, "%s" % self.bucket,  "%s/install_Temp/%s"  % (self.key,filenames))
				s3.upload_file(local_filename, "%s" % self.bucket, "%s/install_Archive/%s"  % (self.key,filenames))

				# Delete file in local
				os.remove(local_filename)      
				print '\n' + 'Successfully uploaded 1 install file into S3 and also deleted in local'
			except Exception as e:	
				print '\n'+'Had error uploading install data to S3'
				print logging.warning(e)
				sys.exit(-1)
			...
			for filename in filenames:            
				local_filename = os.path.join(local_path, filename)
				try:	
					s3.upload_file(local_filename, "%s" % self.bucket,  "%s/install_Temp/%s"  % (self.key,filename))
					s3.upload_file(local_filename, "%s" % self.bucket, "%s/install_Archive/%s"  % (self.key,filename))
					i = i + 1
					# Delete file in local
					os.remove(local_filename)
				except Exception as e:	
			...
			print '\n'+'There might not be install file in local, please check'
			sys.exit(-1)


			print '\n' + 'Successfully upload ',i,' install files into S3'
